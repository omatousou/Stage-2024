[~, ~,~] = Anatole_spectro_mirrror(43, spectro,3);
...
for i =peaks
    ...
    i2 = asin(nair/neff*sin(deg2rad(i)));
    DE2 = e*sin(deg2rad(i)-i2)/cos(i2);
    
    theta3 = rad2deg(-(DE2)/(e3*(1-nair/nlame3)));
    [~, ~,~] = Anatole_spectro_mirrror(theta3+43, spectro,3);
    fprintf('peak =')
    fprintf('angle mirroire 1 = ')
    [x, y,~] = Anatole_spectro_mirrror(calibration2 + i, spectro,2);
